<script lang="ts">
    ...
    function handleEvent(ev: {
        ...
        if (ev.type === "stage") {
            ...
            logs = [
                ...logs,
                { text: ev.message ?? "", kind: "stage", stage: ev.stage },
            ];
...
<div class="min-h-screen bg-surface text-prose">
    ...
    <header class="border-b border-rim bg-card">
        <div class="mx-auto flex max-w-5xl items-center gap-3 px-6 py-4">
            ...
            <div>
                <h1 class="text-lg font-bold tracking-tight">Яйцегляд</h1>
                <p class="text-xs text-muted">
                    Контроль качества · Компьютерное зрение · AI
                </p>
            </div>
            <div
                class="ml-auto flex items-center gap-2 rounded-full border border-rim bg-surface px-3 py-1.5"
            >
                <span class="size-2 animate-pulse rounded-full bg-green-400"
                ></span>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <section class="flex flex-col gap-4">
                ...
                {#if !previewUrl}
                    ...
                    <label
                        class="group relative flex min-h-52 cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed transition-colors
                            ...
                            : 'border-rim bg-card hover:border-amber-500/50 hover:bg-elevated'}"
                    ...
                    >
                        ...
                        >
                            <div
                                class="rounded-full bg-elevated p-4 transition-transform group-hover:scale-110"
                            >
                                <svg
                ...
                {:else}
                    <!-- Preview -->
                    <div
                        class="relative overflow-hidden rounded-xl border border-rim"
                    >
                        <img
                            src={previewUrl}
                            alt="превью"
                        ...
                        ></div>
                        <div
                            class="absolute bottom-0 left-0 right-0 flex items-center justify-between p-3"
                        >
                ...
                <div class="rounded-xl border border-rim bg-card p-4">
                    ...
                    <div class="space-y-3">
                        {#each [{ n: 1, icon: "📸", name: "Moondream2", desc: "Визуальный анализ скорлупы" }, { n: 2, icon: "🧠", name: "Qwen2.5-1.5B", desc: "Вердикт в формате JSON" }] as step}
                            <div class="flex items-center gap-3">
                                <div
                                    class="flex size-7 shrink-0 items-center justify-center rounded-full border text-xs font-bold transition-colors
									{stage === step.n && analyzing
            ...
            <section class="flex flex-col gap-4">
                ...
                <!-- Лог -->
                <div
                    class="min-h-52 overflow-y-auto rounded-xl border border-rim bg-card p-4 font-mono text-xs"
                >
                    {#if logs.length === 0}
                    ...
                    {:else}
                        <div class="space-y-1.5">
                            {#each logs as log}
                                ...
                                {:else}
                                    <div class="flex items-start gap-2 pl-4">
                                        <span class="mt-px shrink-0 text-muted"
                                            >→</span
                                        >
                            ...
                            {#if analyzing}
                                <div class="pl-4">
                                    <span class="animate-blink text-amber-400"
                                        >_</span
                                    >
                                </div>
                            {/if}
                    ...
                    >
                        ...
                        >
                            ...
                            {#if result.confidence && CONFIDENCE[result.confidence]}
                                <div class="text-right">
                                    ...
                                    <p
                                        class="font-semibold {CONFIDENCE[
                                            result.confidence
                                        ].cls}"
                                    >
                        ...
                        <div class="space-y-4 p-5">
                            ...
                            {#if result.defects_found.length > 0}
                                <div>
                                    <p
                                        class="mb-2 text-xs font-semibold uppercase tracking-widest text-muted"
                                    >
                                        Дефекты
                                    </p>
                            ...
                            {#if result.reasoning}
                                <div>
                                    <p
                                        class="mb-1 text-xs font-semibold uppercase tracking-widest text-muted"
                                    >
                                        Обоснование
                                    </p>
                                    <p
                ...
                {#if errorMsg}
                    ...
                    >
                        <p class="mb-1 text-xs font-semibold text-red-400">
                            Ошибка
                        </p>
                        <p class="text-sm text-muted">{errorMsg}</p>
                        <p class="mt-2 text-xs text-muted/60">
                            Убедитесь, что Python-сервис запущен: <code
                                >python service/main.py</code
                            >
                        </p>
                    </div>
                {/if}
    ...
    <footer class="mt-8 border-t border-rim py-5">
        <p class="text-center text-xs text-muted">
            Moondream2 · Qwen2.5-1.5B · SvelteKit · FastAPI
        </p>
    </footer>
</div>
